...
function mobileUI(resources, opt)
{
	...
	this.init = function(resources){
		...
		$('[data-role="video"]').each(function(index){
			...
			parser.href = that.attr('data-url');
			var vid = '';
			switch(prov.toLowerCase())
			{
				case 'youtube':
				if(parser.hostname.indexOf('youtu.be')>-1)
				{
					vid = parser.pathname.substr(1);
				}
				else
				{
				...
				that.find('img').on('click', function(){
					...
					var h = that2.height();
					var rat = w/h;
					that.html('<embed allowfullscreen="true" allowscriptaccess="always" type="application/x-shockwave-flash" src="https://www.youtube.com/v/'+vid+'&autoplay=1" name="plugin" style="height:'+h+'px; max-width:inherit; width:100%; margin:auto">');
				});
				break;
			}		
		});
		...
		$(document).on('click', '[data-role="navbar"][data-rel="swipe-page"] a', function(){
			// set siblings
			var that1 = $(this);
		...
		$(document).on('change', '[data-rel="dynamic-select"]', function(){
		...
		$(document).on('change', '[data-rel="check-all"]', function(){
			var _that = $(this);
		...
		$(document).on('keyup', '[data-role="autocomplete"]', function(){
			var _this = $(this);
				...
				if(source.indexOf('[') == 0)
				{
					...
					thisPlugin.matchArray(data, val, function(res){
						...
						for(i in res)
						{
							...
							$('.mui-autocomplete > ul > li:last-child a').attr('data-value', res[i].v).attr('data-label', res[i].l).bind('click', function(){
					...
					$.ajax({
						...
						'success':function(res){
							...
							for(i in res)
							{
								var html = '<li><a href="#">'+res[i]['l']+'</a></li>';
								$('.mui-autocomplete > ul').append(html);
								$('.mui-autocomplete > ul > li:last-child a').attr('data-val', res[i].v).bind('click', function(){
		...
		if($.inArray('slideshow', arrres) != -1)
		{
			$('[data-role="slideshow"]').each(function(index){
				...
				that.find('.data-slideshow-nav-next a').unbind('click');
				that.find('.data-slideshow-nav-next a').bind('click', function(event){
					var _width = parseFloat(that.width());
				...
				that.find('.data-slideshow-nav-prev a').unbind('click');
				that.find('.data-slideshow-nav-prev a').bind('click', function(event){
					var _width = parseFloat(that.width());
					var _cs = parseInt(that.attr('data-active-slide')) || 1;
					var _fs = _cs;
					var _ss = _cs-1;
		...
		$('[data-filter="true"]').each(function(index){
			...
			fob.bind('change keyup', function(index){
				var val = $(this).val() || '';
				if(val.length > 0)
				{
	...
	this.showPopUp = function(_section)
	{
		...
		if(title!='')
		{
			...
			if(_cls)
			{			
				$('[data-role="popup"][data-section="'+_section+'"] .mui-popup-closer').bind('click', function(){
